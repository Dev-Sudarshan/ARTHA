    ...
    def run(self, image_path: str) -> PipelineResult:
        ...
        try:
            ...
            if canon.success:
                ...
                raise RuntimeError(canon.error)
        except Exception as exc:
            print(f"[WARN] Canonical normalization failed ({exc}); falling back to raw image")
            ...
            if img is None:
                ...
                return result
            canonical_img = img
            warp_metadata = {"fallback": "raw_image"}

        result.warp_metadata = warp_metadata
        t1_end = time.time()
